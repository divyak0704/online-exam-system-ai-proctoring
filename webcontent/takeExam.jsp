...
body { font-family: 'Plus Jakarta Sans', sans-serif; background: var(--bg-dark); color: var(--text-main); min-height: 100vh; overflow-x: hidden; }
...
/* UI Toast */
#toast-container { position: fixed; top: 88px; left: 50%; transform: translateX(-50%); z-index: 2000; display: flex; flex-direction: column; gap: 8px; }
.toast {
  ...
  box-shadow: 0 12px 32px rgba(0,0,0,0.5); animation: toastIn 0.3s cubic-bezier(0.16,1,0.3,1) forwards; max-width: 400px;
}
...
<div id="lockout-overlay">
  <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="var(--danger)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-bottom:16px"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
...
function startTimer(){
  const iv = setInterval(() => {
    ...
    totalTime--;
    if(totalTime < 0){ clearInterval(iv); showToast('⏱ Time expires. Auto-submitting...', 'danger'); setTimeout(() => document.getElementById('examForm').submit(), 2000); }
  }, 1000);
}
startTimer();

function addWarning(type, extra = {}){
  warningCount++; document.getElementById('warningCount').textContent = warningCount;
  showToast('⚠ AI Flag: ' + violationMessage(type));
  sendViolation(type, extra);
  if(warningCount >= MAX_WARNINGS){ document.getElementById('lockout-overlay').classList.add('show'); setTimeout(() => document.getElementById('examForm').submit(), 3000); }
}

function violationMessage(type){
  const m = { FACE_MISSING:'Face obscured', MULTIPLE_FACES:'Secondary entity detected', HEAD_MOVEMENT:'Extreme head deviation', EYE_GAZE:'Gaze out of bounds', OBJECT_DETECTED:'Restricted item identified', AUDIO_NOISE:'Audio threshold breached', TAB_SWITCH:'Environment focus lost', WINDOW_SWITCH:'Hardware focus lost', SCREENSHOT:'Routine capture' };
...
async function sendViolation(type, extra = {}){
  ...
  try { await fetch('LogViolationServlet', { method: 'POST', body: params }); } catch(e){}
}
...
function setBadge(id, state){ document.getElementById(id).className = `status-badge ${state}`; }
setBadge('st-tab', 'ok');

let firstBlur = true;
document.addEventListener('visibilitychange', () => { if(document.hidden){ setBadge('st-tab', 'bad'); addWarning('TAB_SWITCH'); } else setBadge('st-tab', 'ok'); });
window.addEventListener('blur', () => { if(firstBlur){ firstBlur = false; return; } setBadge('st-tab', 'bad'); addWarning('WINDOW_SWITCH'); });
window.addEventListener('focus', () => setBadge('st-tab', 'ok'));
document.addEventListener('contextmenu', e => e.preventDefault());
document.addEventListener('keydown', e => { if(e.key === 'F12' || (e.ctrlKey && ['c','v','u','s'].includes(e.key.toLowerCase()))) e.preventDefault(); });

const video = document.getElementById('camVideo'); const canvas = document.getElementById('camCanvas');
async function startCamera(){
  try{ const s = await navigator.mediaDevices.getUserMedia({video:true,audio:true}); video.srcObject = s; await new Promise(r => video.onloadedmetadata = r); canvas.width = video.videoWidth; canvas.height = video.videoHeight; return s; } catch(e){ showToast('❌ Camera feed required.'); return null; }
}
...
async function runFaceChecks(){
  try{
    const dets = await faceapi.detectAllFaces(video, new faceapi.TinyFaceDetectorOptions({scoreThreshold:0.4})).withFaceLandmarks(true);
    const ctx = canvas.getContext('2d'); ctx.clearRect(0,0,canvas.width,canvas.height);
    if(dets.length===0){ setBadge('st-face','bad'); if(!fM){addWarning('FACE_MISSING');fM=true;} } else { setBadge('st-face','ok'); fM=false; }
    if(dets.length>1) { setBadge('st-multi','bad'); if(!mF){addWarning('MULTIPLE_FACES');mF=true;} } else { setBadge('st-multi',dets.length===1?'ok':'off'); mF=false; }
    
    if(dets.length>=1){
      const lm = dets[0].landmarks;
      const nose = lm.getNose()[3], lE=lm.getLeftEye()[0], rE=lm.getRightEye()[3], emX=(lE.x+rE.x)/2;
      ...
      if(!headOk && !hM){ addWarning('HEAD_MOVEMENT', {head_pose: angle.toFixed(0)+'deg'}); hM=true; } else if(headOk) hM=false;

      const eyeOffset = pts => { const minX=Math.min(...pts.map(p=>p.x)), maxX=Math.max(...pts.map(p=>p.x)), midX=(pts[0].x+pts[3].x)/2, irisX=(pts[1].x+pts[2].x)/2, r=maxX-minX; return r>0?Math.abs(irisX-midX)/r:0; };
      const off = (eyeOffset(lm.getLeftEye()) + eyeOffset(lm.getRightEye()))/2;
      const gazeOk = off <= 0.22;
      setBadge('st-eye', !gazeOk ? 'bad' : 'ok');
      if(!gazeOk && !eG){ addWarning('EYE_GAZE', {eye_gaze: 'offset='+off.toFixed(2)}); eG=true; } else if(gazeOk) eG=false;

      const b = dets[0].detection.box; ctx.strokeStyle = (!headOk || !gazeOk)?'rgba(239,68,68,0.8)':'rgba(16,185,129,0.5)'; ctx.lineWidth=2; ctx.strokeRect(b.x,b.y,b.width,b.height);
    }
  }catch(e){}
}
...
async function loadCocoModel(){ coco = await cocoSsd.load(); setBadge('st-obj','ok'); }
async function runObjectDetection(){
  if(!coco) return;
  try{
    const p = await coco.detect(video); const f = p.filter(x=>BANNED.has(x.class) && x.score>0.55);
    if(f.length>0){ setBadge('st-obj','bad'); if(!oW){ addWarning('OBJECT_DETECTED',{object_detected:f.map(x=>x.class).join(',')}); oW=true; } } else { setBadge('st-obj','ok'); oW=false; }
  }catch(e){}
}

let aW=false;
async function startAudioMonitor(s){
  if(!s){setBadge('st-audio','off'); return;}
  try{
    const ctx = new (window.AudioContext||window.webkitAudioContext)(); const src=ctx.createMediaStreamSource(s); const an=ctx.createAnalyser(); an.fftSize=512; src.connect(an); const buf=new Float32Array(an.fftSize);
    setBadge('st-audio','ok');
    setInterval(()=>{
      an.getFloatTimeDomainData(buf); let sm=0; buf.forEach(v=>sm+=v*v); const rms=Math.sqrt(sm/buf.length);
      if(rms>AUDIO_THRESHOLD){ setBadge('st-audio','bad'); if(!aW){addWarning('AUDIO_NOISE'); aW=true; setTimeout(()=>aW=false,8000);} } else setBadge('st-audio','ok');
    },1500);
  }catch(e){setBadge('st-audio','off');}
}

async function captureScreenshot(){
  const snap = document.createElement('canvas'); snap.width=320; snap.height=240; snap.getContext('2d').drawImage(video,0,0,320,240);
  try{ await fetch('ScreenshotServlet', {method:'POST', body:new URLSearchParams({'screenshot':snap.toDataURL('image/png'), 'examId':'<%=examId%>'})}); }catch(e){}
}

(async function init(){
  const s = await startCamera();
  let e=0; setInterval(()=>{ e++; if(e%SCREENSHOT_EVERY===0) captureScreenshot(); },1000);
  try{ await loadFaceModels(); setInterval(runFaceChecks, FACE_CHECK_EVERY); }catch(x){}
  try{ await loadCocoModel(); setInterval(runObjectDetection, OBJECT_CHECK_EVERY); }catch(x){}
